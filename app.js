...
class ScheduleApp {
    ...
    drawSchedule(xid) {
        ...
        course.classes.find((item) => item.code === xid.classCode).schedules.forEach((schedule) => {
            ...
            scheduleDiv.style.height = `calc(${Number.parseInt(timeDifference / 60) * 4}rem + ${(timeDifference % 60) / 60 * 4}rem - 1px)`;
            scheduleDiv.style.marginTop = `calc(-2rem + ${(schedule.minuteFrom % 60) / 60 * 4}rem + 1px)`;
